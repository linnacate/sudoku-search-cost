/* verify_thm.c — 独立复核定理 1 与引理 1
 * 定理 1: crit(i)>0 <=> minF(i)==2
 * 引理 1: 不动点上 freed(u,d) >= 2 (对空格 i, d in C(i), u in unit(i))
 */
...
int main(int argc,char**argv){
    ...
    printf("\n  crit>0 且 minF==2 : %ld\n", ({
        long s=0; for(int c=1;c<10;c++) s+=TBL[c][2]; s; }));
    printf("  crit=0 且 minF!=2 : %ld\n", ({
        long s=0; for(int m=0;m<10;m++) if(m!=2) s+=TBL[0][m]; s; }));
    printf("  非对角块 (crit>0,minF!=2): %ld\n", ({
        long s=0; for(int c=1;c<10;c++) for(int m=0;m<10;m++) if(m!=2) s+=TBL[c][m]; s; }));
    printf("  非对角块 (crit=0,minF==2): %ld\n", TBL[0][2]);
    return 0;
}
